...
(function () {
    const MODELS = {
        'warehouses': {
            ...
            listApi: '/api/get_warehouses/?page_size=1000',
            api: '/api/warehouses/'
        },
        'cargo-types': {
            fields: ['name', 'description'],
            listApi: '/api/get_cargo_types/?page_size=1000',
            api: '/api/cargo-types/'
        },
        'cargo-statuses': {
            fields: ['name', 'description'],
            listApi: '/api/get_cargo_statuses/?page_size=1000',
            api: '/api/cargo-statuses/'
        },
        'packaging-types': {
            fields: ['name', 'description'],
            listApi: '/api/get_packaging_types/?page_size=1000',
            api: '/api/packaging-types/'
        }
    };
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        try {
            COMPANIES = await fetchJson('/api/get_companies/?page_size=1000').then(r => r.results ?? r);
        } catch (e) {
            COMPANIES = [];
        }
        ...
        if (btnCompany) {
            btnCompany.addEventListener("click", async () => {
                const companyId = window.USER_COMPANY_ID;   // зададим в шаблоне
                const data = await fetch(`/api/company/${companyId}/`).then(r => r.json());
    ...
    `;
        ...
        for (const f of fields) {
            ...
            if (f === 'description' || f === 'address') {
                ...
                emptyOpt.value = '';
                emptyOpt.textContent = '-- выбрать компанию --';
...
function openCompanyModal(data) {
    const html = `
      <div class="modal">
        <div class="modal-header">Компания</div>
        <div class="modal-body">
          ${companyField("Название", "name", data.name)}
          ${companyField("ИНН", "inn", data.inn)}
          ${companyField("ОГРН", "ogrn", data.ogrn)}
          ${companyField("Юр. адрес", "legal_address", data.legal_address)}
          ${companyField("Факт. адрес", "actual_address", data.actual_address)}
          ${companyField("Телефон", "phone", data.phone)}
          ${companyField("Email", "email", data.email)}
        </div>
        <div class="modal-footer">
          <button class="btn btn-cancel" data-modal-close>Отмена</button>
          <button class="btn btn-save" id="save-company">Сохранить</button>
        </div>
      </div>
    `;

    const inst = openModal({html, modalName: "company"});

    inst.modal.querySelector("#save-company").onclick = async () => {
        const payload = {};
        inst.modal.querySelectorAll("input").forEach(el => payload[el.name] = el.value);
...
function companyField(label, name, value) {
    return `
        <div class="modal-row">
            <label>${label}</label>
